<section id="hero" class="grid-bg relative overflow-hidden">
    <div class="mx-auto flex min-h-[85vh] max-w-5xl flex-col justify-center px-4 py-20">
        <div class="max-w-3xl rounded-lg border border-term-border bg-term-panel/60 p-6 shadow-2xl shadow-black/40 sm:p-8">
            <!-- Barre de fenêtre -->
            <div class="mb-6 flex items-center gap-1.5 border-b border-term-border pb-3" aria-hidden="true">
                <span class="h-3 w-3 rounded-full bg-red-500/80"></span>
                <span class="h-3 w-3 rounded-full bg-yellow-500/80"></span>
                <span class="h-3 w-3 rounded-full bg-term-green"></span>
                <span class="ml-3 text-xs text-term-dim"><?= e($profile['handle'] ?? 'user') ?>@<?= e($profile['host'] ?? 'portfolio') ?>: ~/whoami</span>
            </div>

            <p class="text-sm text-term-dim">
                <span class="text-term-green">$</span> whoami
            </p>

            <h1 class="mt-3 text-4xl font-extrabold text-term-bright text-glow sm:text-6xl">
                <?= e($profile['name'] ?? 'Portfolio') ?>
            </h1>

            <p class="mt-3 text-lg text-term-muted sm:text-2xl">
                <span class="text-term-dim">&gt;</span> <?= e($profile['title'] ?? '') ?>
            </p>

            <p class="mt-6 text-sm text-term-dim">
                <span class="text-term-green">$</span> ./run --tagline
            </p>
            <p class="mt-2 min-h-[1.75rem] text-base text-term-bright type-cursor sm:text-lg"
               data-type='<?= e(json_encode($profile['taglines'] ?? [$profile['title'] ?? ''], JSON_UNESCAPED_UNICODE)) ?>'></p>

            <div class="mt-8 flex flex-wrap items-center gap-4 text-sm">
                <a href="#projects"
                   class="rounded border border-term-green bg-term-green/10 px-4 py-2 text-term-bright transition-colors hover:bg-term-green hover:text-term-bg">
                    ls ./projects
                </a>
                <a href="#contact"
                   class="rounded border border-term-border px-4 py-2 text-term-muted transition-colors hover:border-term-green hover:text-term-bright">
                    ./contact.sh
                </a>
                <?php if (!empty($profile['available'])): ?>
                    <span class="flex items-center gap-2 text-xs text-term-green">
                        <span class="h-2 w-2 animate-pulse rounded-full bg-term-green"></span>
                        disponible
                    </span>
                <?php endif; ?>
            </div>
        </div>
    </div>
</section>
